...
<div id="login-page">
	...
	include "koneksi.php";
	$id = $_GET['id'];
	$query_mysql = mysqli_query($koneksi, "SELECT * FROM nilai WHERE id='$id'")or die(mysqli_error($koneksi));
	$nomor = 1;
	while($data = mysqli_fetch_array($query_mysql)){
	?>
    <form class="form-login" method="post" action="edit-nilai.php">
        <h2 class="form-login-heading"> <span class="glyphicon glyphicon-lock"></span> Edit Data Nilai </h2>
        <center>
        ...
        <div class="login-wrap">
          ...
          <br />
          <input name="mapel" id="mapel" type="input" class="form-control" autocomplete="off" placeholder="Masukan Mapel" required value="<?php echo $data['mapel'] ?>">
          <br />
          <input name="nilai1" id="nilai1" type="input" class="form-control" autocomplete="off" placeholder="Masukan Nilai Pengetahuan" required value="<?php echo $data['nilai1'] ?>">
          <br />
          <input name="nilai2" id="nilai2" type="input" class="form-control" autocomplete="off" placeholder="Masukan Nilai Keterampilan" required value="<?php echo $data['nilai2'] ?>">
          <br />
          <button class="btn btn-lg btn-info btn-block" type="submit">Simpan Data</button>
          <hr>
          <div class="login-social-link centered">
                        <a href="data_nilai.admin.php" class="btn btn-success" type="submit"><i class="fa fa-user"></i> KEMBALI KE DATA SISWA </a>
                    </div>
